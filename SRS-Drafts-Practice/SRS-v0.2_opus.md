...
> 시스템을 구성하는 큰 덩어리(컴포넌트)들이 블록처럼 어떻게 맞춰지는지 보여주는 전체 구조도입니다.

```mermaid
flowchart TB
    subgraph Client["🖥️ 클라이언트 계층"]
        MobileWeb["📱 모바일 웹 앱\n(반응형 SPA)"]
        KakaoWebView["💬 카카오 내장 브라우저\n(공유 카드 랜딩)"]
    end
    ...
    subgraph Core["⚙️ 핵심 서비스 계층"]
        CalcEngine["🔢 Super-Calc Engine\n- 1일 단가 환산\n- 다채널 병렬 조회\n- 최종가 산출\n(p95 ≤ 2s)"]
        BadgeService["🏷️ Badge Service\n- MFDS 뱃지 판정\n- 공전 원문 1:1 매칭\n- 금지 표현 필터\n(p95 ≤ 1s)"]
        ShareService["📤 Share Card Service\n- OG 이미지 생성\n- 카카오 Link 연동\n- 폴백 처리\n(p95 ≤ 1.5s)"]
        ReportService["📋 Error Report Service\n- 오류 제보 접수\n- 스팸/중복 필터\n- SLA 48h 추적"]
        LabelService["🏷️ Label Archive Service\n- 원본 라벨 제공\n- 출처 정보 관리"]
        TransService["🌐 용어 번역 서비스\n- 전문→일상어 번역\n- 커버리지 ≥ 95%"]
    ...
    %% 서비스 간 내부 호출
    BadgeService --> TransService
    ShareService --> CalcEngine
    ...
    ReportService --> DB
    LabelService --> DB
    LabelService --> ObjectStorage
    ShareService --> ObjectStorage
    SearchService --> DB
...
| **REQ-NF-026** | 성분 DB 오류 제보가 일 5건 초과 시 긴급 대응 프로세스를 가동한다. | 일 5건 초과 시 긴급 대응 | Notion/Jira 보드 자동 알림 | PRD 5-4 |
...
erDiagram
    USER {
        ...
        string persona_type "C1, C2, A2, E2"
    ...
    PRODUCT {
        ...
        string source_channel "NOT NULL (iHerb/쿠팡/네이버)"
        string original_url "NOT NULL"
    }

    INGREDIENT {
        string ingredient_id PK
        string standard_name "NOT NULL (정규화명)"
        string common_name "일상어 번역명"
        float amount_per_serving "NOT NULL"
        string unit "NOT NULL (mg/IU/mcg)"
        string mfds_status "NOT NULL (APPROVED/CAUTION/NOT_APPROVED/NOT_LISTED)"
        string mfds_claim "식약처 공전 원문"
    }

    PRICE_SNAPSHOT {
        string snapshot_id PK
        string product_id FK
        float original_price "NOT NULL"
        string currency "NOT NULL (KRW/USD)"
        float exchange_rate "외화인 경우"
        float shipping_fee "DEFAULT 0"
        float customs_fee "DEFAULT 0"
        float daily_cost_krw "NOT NULL (1일 단가)"
        datetime captured_at "NOT NULL"
    }

    BADGE {
        string badge_id PK
        string ingredient_id FK
        string badge_type "NOT NULL (APPROVED/CAUTION/NOT_APPROVED)"
        string badge_label "NOT NULL (공전 원문 래핑)"
        string evidence_source "NOT NULL"
        string evidence_url
    }
    ...
    ERROR_REPORT {
        ...
        string reported_value "NOT NULL"
        string correct_value "NOT NULL"
        string status "NOT NULL (SUBMITTED/REVIEWING/RESOLVED/REJECTED/SPAM)"
        datetime reported_at "NOT NULL"
...
#### 6.2.10 Class Diagram (클래스 다이어그램)

> 시스템을 구성하는 핵심 요소(클래스)들이 각각 어떤 데이터를 가지고 있고, 서로 어떤 역할을 주고받는지 정리한 뼈대 설계도입니다. 개발자가 코드를 짤 때 가장 많이 참고하는 그림입니다.
...
classDiagram
    class Product {
        ...
        +getPriceSnapshots() List~PriceSnapshot~
        +getLabels() List~LabelArchive~
        +getErrorReports() List~ErrorReport~
    }
    ...
    class PriceSnapshot {
        ...
        +calcDailyCostKRW() Float
        +calcFinalPrice() Float
        +applyExchangeRate(rate: Float) void
    }
    ...
    class Badge {
        ...
        +checkForbiddenExpressions() Boolean
    }
    ...
    class User {
        ...
        -String persona_type
        -DateTime created_at
        +submitReport(report: ErrorReport) void
        +getReports() List~ErrorReport~
    }

    class SuperCalcEngine {
        +calcDailyCost(ingredient: String, channels: List) List~PriceSnapshot~
        -fetchPricesParallel(channels: List) List~RawPrice~
        -applyExchangeRate(prices: List, rate: Float) List~PriceSnapshot~
        -sortByDailyCost(snapshots: List) List~PriceSnapshot~
        -handleChannelFailure(channel: String) PartialResult
    }

    class BadgeService {
        +getBadges(ingredientId: String) List~Badge~
        -queryMFDS(ingredientName: String) MFDSResponse
        -judgeBadgeType(mfdsData: MFDSResponse) BadgeType
        -validateBadgeLabel(label: String) Boolean
        -cacheBadge(badge: Badge, ttl: Duration) void
    }

    class ShareCardService {
        +createShareCard(comparisonData: Object) ShareCard
        -generateOGImage(data: Object) String
        -sendViaKakao(card: ShareCard) Result
        -fallbackToURLCopy(card: ShareCard) String
    }

    class ErrorReportService {
        +submitReport(report: ErrorReport) Result
        -filterSpamDuplicate(report: ErrorReport) Boolean
        -notifyAdmin(report: ErrorReport) void
        -notifyReporter(report: ErrorReport) void
        -grantReward(userId: String) void
    }

    class LabelArchiveService {
        +getLabels(productId: String) List~LabelArchive~
        +getEvidenceSources(productId: String) List~Source~
    }
    ...
    Product "1" --> "*" PriceSnapshot : has price history
    Product "1" --> "*" LabelArchive : has labels
    Product "1" --> "*" ErrorReport : receives reports
    Ingredient "1" --> "*" Badge : has badges
    User "1" --> "*" ErrorReport : submits

    %% 서비스 → 엔터티 의존
    SuperCalcEngine ..> Product : queries
    SuperCalcEngine ..> PriceSnapshot : creates
    BadgeService ..> Ingredient : queries
    BadgeService ..> Badge : creates/caches
    ShareCardService ..> PriceSnapshot : reads
    ShareCardService ..> Badge : reads
    ErrorReportService ..> ErrorReport : manages
    LabelArchiveService ..> LabelArchive : queries

    %% 열거형 사용
    Badge --> BadgeType
    ErrorReport --> ReportStatus
```
