...
본 문서는 실버케어 Phase A MVP 및 Phase B 확장 API에 대한 Software Requirements Specification이다. 본 SRS는 [REF-01]의 PRD v0.5를 유일한 제품 요구 원천으로 사용하여, B2B2C 돌봄 기기 파트너가 텍스트 기반 시니어 대화 API, 정서/위험 신호 태깅 API, PoC 리포트 API, Partner Console, 운영 모니터링 및 향후 센서 기반 통합 돌봄 API를 구현·검증할 수 있도록 요구사항을 정의한다.
...
| ASSUMP-001 | Assumption | PRD v0.5는 상세 OpenAPI type, field length, pagination 방식을 확정하지 않는다. SRS는 PRD 필드를 기준으로 baseline schema를 정의하며 최종 OpenAPI는 별도 산출물로 확정한다. | [REF-01] 4, 9 | OpenAPI backlog 생성 |
| ASSUMP-002 | Assumption | PRD v0.5는 RPO/RTO 수치를 제공하지 않는다. 본 SRS는 Phase A PoC 운영 기준선을 제안하며 production GA 전 제품·운영 책임자 승인이 필요하다. | [REF-01] 5-3, 9 | 운영 runbook 승인 기록 |
...
| RPO | Recovery Point Objective. 장애 시 허용 가능한 데이터 손실 시간. PRD v0.5 미확정, 본 SRS에서 PoC baseline을 둔다. |
| RTO | Recovery Time Objective. 장애 후 서비스 복구 목표 시간. PRD v0.5 미확정, 본 SRS에서 PoC baseline을 둔다. |
...
| AOS | Adjusted Opportunity Score. PRD v0.5에서 수치가 제공되지 않았으므로 본 SRS의 요구 우선순위 산정에는 사용하지 않는다. |
| DOS | Discovered Opportunity Score. PRD v0.5에서 수치가 제공되지 않았으므로 본 SRS의 요구 우선순위 산정에는 사용하지 않는다. |
...
|---|---|---|---|
| REF-01 | `실버케어초안_PRD_v0.5.md` | v0.5 / 2026-04-28 | 본 SRS의 유일한 제품 요구 Source of Truth |
...
| REQ-FUNC-050 | Developer Portal은 Swagger 문서, enum 정의, 표준 오류 코드, cURL/Python 샘플 코드를 조회할 수 있어야 한다. | Story B2B-DEV AC-1; F0 | Must | Given 개발자가 API Docs를 열람, When endpoint를 선택, Then request/response schema, enum, error code, cURL/Python sample이 표시된다. | TC-FUNC-050 |
...
| REQ-FUNC-055 | Web API Playground는 현재 request payload를 기준으로 cURL 및 Python snippet을 생성해야 한다. | Story B2B-DEV AC-1; F0 | Must | Given 유효한 request payload, When 사용자가 snippet 생성을 선택, Then 동일 endpoint와 payload를 사용하는 cURL 및 Python sample이 생성된다. | TC-FUNC-055 |
...
| REQ-NF-003 | Availability / DR | Phase A PoC의 API 및 주요 운영 데이터 저장소는 RPO 24시간 이하, RTO 4시간 이하를 운영 baseline으로 가져야 한다. 해당 수치는 PRD v0.5 미확정 항목이므로 production GA 전 승인되어야 한다. | RPO <= 24h, RTO <= 4h | PRD 5-3 backup policy; ASSUMP-002 | Restore drill, runbook approval |
...
| REQ-NF-031 | Cost / FinOps | 정확한 원화 단가는 PRD v0.5에서 확정하지 않으므로 시스템은 가격표 없이도 활성 디바이스·호출량·초과 사용 가능성 지표를 산출해야 한다. | No KRW price required for PoC report generation | PRD 1-5 | PoC report test |
...
본 필드 명세는 PRD v0.5의 API 및 데이터 정책을 구현 가능한 baseline schema로 변환한 것이다. 물리 DB의 문자열 길이, partition strategy, storage engine은 상세 설계에서 확정하되, 아래의 key, nullable, privacy, retention 의미는 변경하지 않아야 한다.
...
flowchart TB
    ...
    subgraph SilverCare["SilverCare API Platform"]
        ...
        Gateway["API Gateway<br/>Auth, Rate Limit, Request ID"]
    ...
    subgraph External["External Providers"]
        LLM["External/Internal LLM Engine"]
    ...
    Consent --> OperationalDB
    Memory --> OperationalDB
    Chat --> EventStore
    Analysis --> EventStore
    Metering --> EventStore
    Report --> OperationalDB
    Ops --> EventStore
    Ops --> Evaluation
...
보호자용 웹/앱 또는 기관용 웹/앱을 실버케어가 직접 제공하려면 본 SRS만 수정해서는 안 된다. PRD v0.5에서 해당 앱은 Out-of-Scope로 정의되어 있으므로, 제품 범위 변경은 PRD를 먼저 수정한 뒤 SRS와 하위 산출물을 갱신해야 한다.
