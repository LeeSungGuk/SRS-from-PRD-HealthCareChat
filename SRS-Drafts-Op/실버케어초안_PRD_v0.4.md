# 실버케어초안_PRD.v0.4.md
- **Owner 팀**: 이성국
- **최종 업데이트**: 2026-04-25 (사용자 스토리 및 SRS 전환성 보강)
- **문서 버전**: v0.4 (v0.3 API 플랫폼 B2B2C 모델 유지 + 사용자 스토리/AC 확장)
...
| P0 Core | 개발자 경험 | Swagger 문서, cURL/Python 샘플 요청/응답, 표준 오류 코드, sandbox elder profile |
...
| P0 Support | 운영 모니터링 | 요청 로그, `request_id`, 지연시간, 오류율, 안전 정책 발동 여부, PII 마스킹 로그 |
...
| 보호자용/기관용 완성 앱 직접 제공 | MVP는 API 검증이 목적이며, 앱 UI는 파트너 또는 후속 Phase에서 결정 |
...
|---|---|---|
| **B2B-PM. 제품/사업 담당자** | "기기를 팔아야 하는데 차별화된 AI 돌봄 경험이 없다." | PoC 지표로 도입 효과를 검증하고 구독형 API로 빠르게 상품화 |
| **B2B-DEV. 연동 개발자** | "AI 모델, 프롬프트, 안전장치, 분석 API를 직접 만들 시간이 없다." | API Key, Swagger, 샘플 코드만으로 빠른 연동 가능 |
...
| **D. 플랫폼 운영자** | "AI 응답 품질, 위험 이벤트, 개인정보 처리가 안정적으로 동작하는지 봐야 한다." | 요청 로그, 지연시간, 오류율, 안전 정책 발동 기록을 비식별 기반으로 모니터링 |
...
| # | AC (Given / When / Then) |
|:---:|---|
| AC-1 | **Given** 유효한 파트너 계정이 있을 때, **When** 개발자 포털에 접속하면, **Then** API Key 발급, Swagger 문서, 샘플 요청/응답, 에러 코드 표를 확인할 수 있어야 한다. |
...
| AC-4 | **Given** sandbox 환경에서 테스트 중일 때, **When** 개발자가 샘플 elder profile과 샘플 발화를 사용하면, **Then** 실제 개인정보 없이도 정상 응답, 안전 응답, 오류 응답을 모두 재현할 수 있어야 한다. |
...
| # | AC (Given / When / Then) |
|:---:|---|
| AC-5 | **Given** PoC가 시작되었을 때, **When** 파트너가 리포트 API 또는 CSV 집계를 조회하면, **Then** 일별 활성 어르신 수, 인당 대화 횟수, p95 지연시간, 오류율, 위험 신호 발생 수를 확인할 수 있어야 한다. |
| AC-6 | **Given** PoC 종료 시점이 되었을 때, **When** 파트너가 성과 데이터를 요청하면, **Then** 개인 식별정보를 제외한 집계 리포트를 CSV 또는 JSON 형태로 받을 수 있어야 한다. |
...
| # | AC (Given / When / Then) |
|:---:|---|
| AC-18 | **Given** 운영자가 요청 로그를 조회할 때, **When** 로그 화면 또는 로그 API를 열람하면, **Then** 원문 PII는 마스킹된 상태로 표시되고 `tenant_id`, `request_id`, 지연시간, 오류 코드, 안전 정책 발동 여부를 확인할 수 있어야 한다. |
...
| AC-20 | **Given** 위험 징후 탐지 결과에 대한 오탐/미탐 리뷰가 필요할 때, **When** 운영자가 리뷰 데이터를 추출하면, **Then** 개인정보를 제거한 평가용 데이터셋과 라벨링 결과를 분리해서 관리할 수 있어야 한다. |
...
| ID | API/기능 | 설명 | 타겟 |
|---|---|---|---|
| **F0** | **Developer Portal / Sandbox** | API Key 발급, Swagger 문서, 샘플 코드, sandbox elder profile, 표준 오류 응답 예제를 제공. | B2B-DEV |
